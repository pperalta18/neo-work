...
| **Hero "Inteligencia"** (el grid de la referencia — **no** es un flujo) | `HeroIntro` ([`hero/HeroIntroVideo.tsx`](../src/remotion/hero/HeroIntroVideo.tsx)) — **render único** 159 f / ~5,3 s, **bucle perfecto**, reglas propias en [`hero-animation.md`](./hero-animation.md) | ✅ construido (v3.1, 2026-06-12) — **simplemente el grid `inteligencia`** (3×2, ruta serpenteante); la animación es la **EMERGENCIA escalonada** de los 6 ítems (patrón `GridEmerge`: disco inicio → flechas → meta), guiada por un **puntito azul diminuto y tenue**; al completarse, **desaparecen en el mismo orden** (FIFO, ola continua) casi inmediatamente, con reposo de costura ~1 f → bucle (`f0≡f158`). v2 «ecosistema vivo» y v3 «línea azul» descartadas. `pnpm run render:hero` |
